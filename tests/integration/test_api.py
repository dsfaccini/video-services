...
class TestAPIIntegration:
    ...
    @pytest.mark.asyncio
    async def test_extract_video_url_invalid_url(
        self, async_client: AsyncClient
    ) -> None:
        """Test extraction with invalid URL format."""
        response = await async_client.get(
            "/api/video/extract-url", params={"url": "not-a-valid-url"}
        )
...
class TestVideoProcessingIntegration:
    ...
    def test_clip_local_video_file(
        self, create_test_video: Callable[[str], str], temp_dir: Path
    ) -> None:
        ...
        try:
            (
                typed_ffmpeg.input("color=c=blue:s=320x240:d=5", f="lavfi")
...
class TestAPIErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_extract_video_url_unsupported_site(
        self, async_client: AsyncClient
    ) -> None:
        """Test extraction from an unsupported site."""
        response = await async_client.get(
            "/api/video/extract-url",
            params={"url": "https://example.com/not-a-video-site"},
        )
        assert response.status_code == 400
