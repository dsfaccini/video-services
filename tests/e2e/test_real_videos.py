...
class TestRealVideoExtraction:
    ...
    @pytest.mark.parametrize(
        "test_data", ACTIVE_TEST_URLS, ids=lambda d: d["description"]
    )
    def test_extract_video_url_from_real_posts(self, test_data):
        ...
        try:
            ...
            if not test_data.get("active", True):
                ...
                pytest.fail(
                    f"Failed to extract video from {test_data['description']}: {str(e)}"
                )

    @pytest.mark.parametrize(
        "test_data", LINKEDIN_VIDEO_URLS, ids=lambda d: d["description"]
    )
    def test_clip_linkedin_videos(self, test_data, temp_dir):
        ...
        try:
            ...
            if os.getenv("SAVE_TEST_VIDEOS"):
                output_path = (
                    temp_dir / f"linkedin_clip_{test_data['url'].split('/')[-2]}.mp4"
                )
            ...
            import tempfile
            log_file = tempfile.mktemp(suffix='.log', prefix='e2e_clip_test_')
            with open(log_file, 'w') as f:
                f.write(f"✓ Successfully clipped video: {len(video_bytes)} bytes\n")
                f.write(f"Test URL: {test_data['url']}\n")
            print(f"✓ Successfully clipped video (size logged to {log_file})")
...
class TestRealVideoAPI:
    ...
    async def test_api_extract_linkedin_video(self, async_client):
        ...
        response = await async_client.get(
            "/api/video/extract-url", params={"url": linkedin_url}
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(
        os.getenv("SKIP_SLOW_E2E") == "1", reason="Skipping slow E2E test"
    )
    async def test_api_clip_real_video(self, async_client):
        ...
        extract_response = await async_client.get(
            "/api/video/extract-url", params={"url": linkedin_url}
        )
        ...
        # Then clip it
        clip_response = await async_client.get(
            "/api/video/clip",
            params={"url": video_url, "start_time": 0.0, "end_time": 2.0},
        )
        ...
        # Log video size to temp file instead of console
        import tempfile
        log_file = tempfile.mktemp(suffix='.log', prefix='e2e_api_clip_')
        with open(log_file, 'w') as f:
            f.write(f"API clipped video: {len(video_bytes)} bytes\n")
            f.write(f"Video URL: {video_url}\n")
        print(f"\nAPI clipped video (size logged to {log_file})")
