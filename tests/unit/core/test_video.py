...
class TestExtractVideoUrl:
    ...
    def test_extract_video_url_success_with_direct_url(self, mock_yt_dlp_success):
        ...
        mock_yt_dlp_success.extract_info.assert_called_once_with(
            "https://x.com/user/status/123456", download=False
        )
    ...
    def test_extract_video_url_yt_dlp_exception(self, mocker):
        ...
        with pytest.raises(
            ValueError, match="Failed to extract video URL: Network error"
        ):
            extract_video_url("https://example.com/post")
...
class TestClipVideo:
    ...
    def test_clip_video_download_failure(self, mocker, temp_dir):
        ...
        with patch("tempfile.NamedTemporaryFile") as mock_temp:
            ...
            with patch("os.path.exists", return_value=False):
                with pytest.raises(
                    ValueError, match="Failed to clip video: Download failed"
                ):
